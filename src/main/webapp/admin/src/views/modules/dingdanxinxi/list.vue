<template>
  <div class="main-content">
		    ...
		    label="姓名">
		     <template slot-scope="scope">
                ...
                <el-button v-if="isAuth('dingdanxinxi','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                                                                <el-button v-if="isAuth('dingdanxinxi','发送文件') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yinlewenjianCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送文件':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送文件') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yinlewenjianCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送文件':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','发送文件') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yinlewenjianCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发送文件':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <yinlewenjian-cross-add-or-update v-if="yinlewenjianCrossAddOrUpdateFlag" :parent="this" ref="yinlewenjianCrossaddOrUpdate"></yinlewenjian-cross-add-or-update>
...
import yinlewenjianCrossAddOrUpdate from "../yinlewenjian/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                                                                searchForm: {
      ...
      addOrUpdateFlag:false,
                  yinlewenjianCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            yinlewenjianCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yinlewenjianCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
        this.$refs.yinlewenjianCrossaddOrUpdate.init(row.id,type);
      });
    },
                payHandler(row){
